<form method="POST" action="{{ $stepUrl('step7') }}" class="js-card-form hcb-js-card-form">
    ...
    <div class="row g-3">
        ...
        <div class="col-12"><h6 class="fw-bold border-bottom pb-1">{{ $getSection('7.1 Application Type') ? $getSection('7.1 Application Type')['title'] : '7.1 Application Type' }}</h6></div>
        <div class="col-md-6">
        ...
        <div class="col-md-6">
            <div class="form-check">
                <input class="form-check-input" type="checkbox" name="extension_scope" value="1" id="decl_ext_scope"
                       @if(old('extension_scope', $declaration->extension_scope ?? false)) checked @endif>
        ...
        <div class="col-12 mt-2"><h6 class="fw-bold border-bottom pb-1">{{ $getSection('7.2 Declarations') ? $getSection('7.2 Declarations')['title'] : '7.2 Declarations' }}</h6></div>

        <div class="col-12">
            <div class="form-check">
                <input class="form-check-input" type="checkbox" name="quality_manual_confirmed" value="1" id="qm_confirmed"
                       @if(old('quality_manual_confirmed', $declaration->quality_manual_confirmed ?? false)) checked @endif>
        ...
        <div class="col-12">
            <div class="form-check">
                <input class="form-check-input" type="checkbox" name="declaration_accepted" value="1" id="decl_accepted"
                       @if(old('declaration_accepted', $declaration->declaration_accepted ?? false)) checked @endif
                       required>
        ...
        <div class="col-12 mt-2"><h6 class="fw-bold border-bottom pb-1">{{ $getSection('7.3 Applicant Fee & Signature') ? $getSection('7.3 Applicant Fee & Signature')['title'] : '7.3 Applicant Fee & Signature' }}</h6></div>

        <div class="col-md-4">
            <label class="form-label">{{ $getLabel('7.3 Applicant Fee & Signature', 'applicant_fee_amount', 'Applicant Fee (PKR)') }}</label>
        ...
        <div class="col-md-4">
            <label class="form-label">{{ $getLabel('7.3 Applicant Fee & Signature', 'signed_by', 'Signed By') }} <span class="text-danger">*</span></label>
            <input class="form-control" name="signed_by"
                   value="{{ old('signed_by', $declaration->signed_by ?? '') }}" required>
        </div>
        <div class="col-md-4">
            <label class="form-label">{{ $getLabel('7.3 Applicant Fee & Signature', 'signed_date', 'Date') }} <span class="text-danger">*</span></label>
    ...
    <!--    @if(!$isLocked)-->
    <!--        <button type="submit" name="final_submit" value="1" class="btn btn-primary btn-sm">Final Submit</button>-->
    <!--    @endif-->
    <!--</div>-->
</form>
